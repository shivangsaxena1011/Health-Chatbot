...
नमस्ते! मैं स्वस्थ AI 2.0 हूँ। आज मैं स्वास्थ्य जागरूकता और लक्षणों को समझने में आपकी क्या मदद कर सकता हूँ?`,
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-slate-50">
      ...
      {/* Message List */}
      <div className="flex-1 overflow-y-auto px-3 sm:px-6 md:px-8 pt-3 sm:pt-4 pb-44 md:pb-36 scroll-smooth overflow-x-hidden">
        <div className="max-w-3xl mx-auto w-full">
          ...
          {messages.length === 1 && (
            <div className="mt-6 sm:mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h4 className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest text-center mb-3 sm:mb-4 flex items-center justify-center">
                <span className="h-px w-6 sm:w-8 bg-slate-200 mr-2 sm:mr-3"></span>
                Quick-Start Health Inquiries
                <span className="h-px w-6 sm:w-8 bg-slate-200 ml-2 sm:ml-3"></span>
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 px-1">
                {suggestedTopics.map((topic, i) => (
                  <button
                    key={i}
                    onClick={() => handleSend(topic.query)}
                    className="p-3 sm:p-3.5 bg-white border border-slate-200 rounded-2xl text-left hover:border-emerald-500 hover:shadow-md hover:shadow-emerald-50 transition-all group flex items-start space-x-3 active:scale-[0.98]"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-slate-800 truncate">{topic.title}</p>
          ...
          {isLoading && (
            <div className="flex justify-start mb-6 mt-4 ml-4 sm:ml-10">
              <div className="flex items-center space-x-2 bg-white px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-2xl rounded-tl-none border border-slate-200 shadow-sm">
                <Loader2 className="w-4 h-4 text-emerald-600 animate-spin" />
                <span className="text-xs font-medium text-slate-600">
                  Retrieving medical evidence & analyzing...
                </span>
              </div>
            </div>
          )}
      ...
      {/* Floating Input Area (Positioned cleanly above MobileNav on small screens) */}
      <div className="absolute bottom-16 md:bottom-0 left-0 right-0 p-2.5 sm:p-4 md:p-6 bg-gradient-to-t from-slate-50 via-slate-50/95 to-transparent pointer-events-none z-20">
        <div className="max-w-3xl mx-auto pointer-events-auto w-full">
          ...
          >
            <div className="relative flex-1 shadow-lg sm:shadow-xl shadow-slate-200/60 rounded-2xl overflow-hidden bg-white border border-slate-200 group-focus-within:border-emerald-500 transition-all ring-0 group-focus-within:ring-4 group-focus-within:ring-emerald-500/10">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about symptoms, medicines, or health tips..."
                className="w-full bg-transparent border-none pl-3.5 sm:pl-6 pr-24 py-3 sm:py-4 text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0"
                disabled={isLoading || isRecording}
              />
              <div className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 flex items-center space-x-1">
                <button
                  type="button"
                  onClick={isRecording ? stopRecording : startRecording}
                  className={`min-w-[40px] min-h-[40px] p-2 sm:p-2.5 rounded-xl transition-all flex items-center justify-center ${
                    isRecording
                      ? 'text-white bg-red-500 animate-pulse'
                      : 'text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 active:bg-slate-100'
                  }`}
                ...
                <button
                  ...
                  className={`min-w-[40px] min-h-[40px] w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
